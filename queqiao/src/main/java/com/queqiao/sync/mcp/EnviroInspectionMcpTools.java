...
public class EnviroInspectionMcpTools {
    ...
    @Tool(name = "get_inspection_ledger", description = "获取指定日期危废仓库巡查台账：含巡检汇总、各摄像头结果与台账记录。不穿透环保小脑，读鹊桥自有库。")
    ...
    @Tool(name = "get_camera_status", description = "获取摄像头状态：指定摄像头名返回最新快照与近 N 天历史；不指定则返回每个摄像头最新一条。")
    ...
    @Tool(name = "get_inspection_summary", description = "获取区间内巡检汇总：在线率、最差记录日、频繁离线摄像头排名。")
    ...
    @Tool(name = "trigger_inspection", description = "触发环保小脑执行一次巡检（操作类，转发环保小脑；不可达时返回友好错误）。")
    ...
    @Tool(name = "download_ledger_docx", description = "下载指定巡检记录的台账 Word 文档（操作类，转发环保小脑；不可达时返回友好错误）。")
